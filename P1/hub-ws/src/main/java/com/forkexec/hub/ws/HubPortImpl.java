...
public class HubPortImpl implements HubPortType {
	...
	public void activateAccount(String userId) throws InvalidUserIdFault_Exception {
		...
		if(!userId.matches(pattern)) throwInvalidUserId(userId);



		try {
			UDDINaming uddiNaming = this.endpointManager.getUddiNaming();
			UDDIRecord record = uddiNaming.lookupRecord("T22_Points1");
	...
	public void loadAccount(String userId, int moneyToAdd, String creditCardNumber)
		...
		try {
			...
			else { moneyToAdd = 0; throwInvalidMoney("Wrong money value to add");}

			UDDINaming uddiNaming = this.endpointManager.getUddiNaming();

			UDDIRecord recordPoints = uddiNaming.lookupRecord("T22_Points1");
			PointsClient pointsClient = new PointsClient(recordPoints.getUrl());
			pointsClient.addPoints(userId, pointsToAdd);

			//UDDIRecord recordCC = uddiNaming.lookupRecord("http://ws.sd.rnl.tecnico.ulisboa.pt:8080/cc");
		...
		catch(InvalidPointsFault_Exception exception) {
			System.out.println("InvalidPointsFault_Exception: " + exception.toString());
		}
	...
	public void ctrlInitFood(List<FoodInit> initialFoods) throws InvalidInitFault_Exception {
		...
		synchronized(hub) {
			...
			while (foodInitIterator.hasNext()) {
				...
				/*HubFood food = new HubFood(new HubFoodId(foodIdAux.getRestaurantId(),foodIdAux.getMenuId()) ,foodAux.getEntree(), foodAux.getPlate(), foodAux.getDessert(),
						foodAux.getPrice(), foodAux.getPreparationTime(), foodInitAux.getQuantity());*/
